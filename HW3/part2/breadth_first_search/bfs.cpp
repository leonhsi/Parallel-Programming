	...
	bool *visited)
{
    ...
    for (int i = 0; i < frontier->count; i++)
    {
		...
		int num_neighbors = end_edge - start_edge;
		int append[num_neighbors];
		int counter = 0;
		...
		if(counter > 0){
			...
			for(int j=0; j<counter; j++){
				new_frontier->vertices[index+j] = append[j];
			}
		}
    }
}
...
void bfs_top_down(Graph graph, solution *sol)
{
    ...
    while (frontier->count != 0)
    {
        ...
        top_down_step(graph, frontier, new_frontier, sol->distances, visited);
